...
Aktueller Stand fuer die Bachelorarbeit. Legacy-Dateien in `src/` und `scripts/` bleiben unveraendert.

## Struktur

- `v2/scripts/` - ausfuehrbare Skripte (Datensammlung, Pipeline)
- `v2/src/` - neuer Python-Code (Module)
- `v2/notebooks/` - Experiment-Notebook + gespeicherte Plots (`charts/`)

## Dataset bauen

High-Elo (Challenger / GM / Master), Queue 420, faire Verteilung ueber alle Regionen,
...
- `--resume` - bestehende CSV weiterfuehren statt neu starten
...
- `--parallel` - Routing-Threads (weniger fair bei kleinem n, schneller bei grossem n)
